...
| 🍸 | **[Bar Layout Optimisation](https://github.com/seanhoet65/bar-layout-optimisation)** | Where should a bartender put the bottles? Builds a co-occurrence network from cocktail recipes, assigns ingredients to three reach zones by graph centrality, then orders them within each zone by 1-D MDS so things poured together sit side by side. Written in R. The part I'd point to: I ran Louvain community detection, and then threw the result out — structurally correct, but the clusters cut across categories a bartender would recognise. |
